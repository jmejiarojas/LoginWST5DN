...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        private String  _URL="http://cibertec201722.somee.com/USUARIO/FrmUsuario.aspx?Accion=V&pLogin={0}&pContrasenia={1}";
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            String URLDatos=_URL.replace("{0}",mEmail);
            URLDatos=URLDatos.replace("{1}",mPassword);
            try
            {_MensajeResultado=Funciones.DescargarPagina(URLDatos);}
            catch (Exception ex)
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success)
            {
                try {JSONObject objResponse = new JSONObject(_MensajeResultado);
                     ...
                     JSONArray ObjListaUsuarios=objResponse.getJSONArray("ObjListaUsuario");
                     if (ObjUsuario.getInt("CodigoUsuario")>0)
                     {Toast.makeText(getApplication(),
                                "Bienvenido:" +ObjUsuario.getString("Nombres")
                                , Toast.LENGTH_LONG).show();

                         Intent oIntento=new Intent(getApplication(),
                                 ListadoUsuario.class);
